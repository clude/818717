...
class Searcher(object):
    ...
    def search(self, query='', sort='', start=0, limit=20):
        ql = sphinxql.search(self.index, 'json')
        if query: ql.keyword(query)
    ...
    def update(self, rows):
        ...
        for row in rows:
            ...
            if self.rc.exists(id_hash):
                ...
                ql, hit_count = replace_ql, check_hit+1
            ...
            self.rc.expire(id_hash, EXPIRE_TTL)
            row['id'] = gid
    ...
    def reindex(self, BATCH_COUNT=50):
        ...
        all_keys = self.rc.keys()
        print '%d rows in redis.'%len(all_keys)

        done = 0
        for page in range(int(math.ceil(float(len(self.rc.keys()))/BATCH_COUNT))):
