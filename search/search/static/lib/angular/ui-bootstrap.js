...
function TabsetCtrl($scope, $element) {
  ...
  ctrl.removeTab = function removeTab(tab) {
    ...
    //Select a new maptab if the maptab to be removed is selected
    if (tab.active && tabs.length > 1) {
      //If this is the last maptab, select the previous maptab. else, the next maptab.
...
<example module="ui.bootstrap">
  <file name="index.html">
    <tabset>
      <maptab heading="Vertical Tab 1"><b>First</b> Content!</maptab>
      <maptab heading="Vertical Tab 2"><i>Second</i> Content!</maptab>
    </tabset>
    <hr />
    <tabset vertical="true">
      <maptab heading="Vertical Tab 1"><b>First</b> Vertical Content!</maptab>
      <maptab heading="Vertical Tab 2"><i>Second</i> Vertical Content!</maptab>
    </tabset>
  </file>
</example>
 */
...
/**
 * @ngdoc directive
 * @name ui.bootstrap.tabs.directive:maptab
 * @restrict EA
 *
 * @param {string=} heading The visible heading, or title, of the maptab. Set HTML headings with {@link ui.bootstrap.tabs.directive:tabHeading tabHeading}.
 * @param {string=} select An expression to evaluate when the maptab is selected.
 * @param {boolean=} active A binding, telling whether or not this maptab is selected.
 * @param {boolean=} disabled A binding, telling whether or not this maptab is disabled.
 *
 * @description
 * Creates a maptab with a heading and content. Must be placed within a {@link ui.bootstrap.tabs.directive:tabset tabset}.
 *
 * @example
<example module="ui.bootstrap">
  <file name="index.html">
    <div ng-controller="TabsDemoCtrl">
      ...
      <br />
      <tabset>
        <maptab heading="Tab 1">First Tab</maptab>
        <maptab select="alertMe()">
          <maptab-heading><i class="icon-bell"></i> Alert me!</maptab-heading>
          Second Tab, with alert callback and html heading!
        </maptab>
        <maptab ng-repeat="item in items"
          heading="{{item.title}}"
  ...
  <file name="script.js">
    function TabsDemoCtrl($scope) {
      ...
      $scope.alertMe = function() {
        setTimeout(function() {
          alert("You've selected the alert maptab!");
        });
      };
    };
  </file>
 ...
 *
 * @description
 * Creates an HTML heading for a {@link ui.bootstrap.tabs.directive:maptab maptab}. Must be placed as a child of a maptab element.
 *
 * @example
<example module="ui.bootstrap">
  <file name="index.html">
    <tabset>
      <maptab>
        <maptab-heading><b>HTML</b> in my titles?!</maptab-heading>
        And some content, too!
      </maptab>
      <maptab>
        <maptab-heading><i class="icon-heart"></i> Icon heading?!?</maptab-heading>
        That's right.
      </maptab>
    </tabset>
  </file>
 ...
 */
.directive('maptab', ['$parse', '$http', '$templateCache', '$compile',
function($parse, $http, $templateCache, $compile) {
  return {
    require: '^tabset',
    restrict: 'EA',
    replace: true,
    templateUrl: 'template/tabs/maptab.html',
    transclude: true,
    scope: {
      heading: '@',
      onSelect: '&select', //This callback is called in contentHeadingTransclude
                          //once it inserts the maptab's content into the dom
      onDeselect: '&deselect'
    },
    controller: function() {
      //Empty controller so other directives can require being 'under' a maptab
    },
    compile: function(elm, attrs, transclude) {
      return function postLink(scope, elm, attrs, tabsetCtrl) {
        ...
        //when this link happens, we're inside a maptab heading.
...
.directive('tabContentTransclude', ['$compile', '$parse', function($compile, $parse) {
  return {
    ...
    link: function(scope, elm, attrs) {
      ...
      //Now our maptab is ready to be transcluded: both the maptab heading area
      //and the maptab content area are loaded.  Transclude 'em both.
  ...
  function isTabHeading(node) {
    return node.tagName &&  (
      node.hasAttribute('maptab-heading') ||
      node.hasAttribute('data-maptab-heading') ||
      node.tagName.toLowerCase() === 'maptab-heading' ||
      node.tagName.toLowerCase() === 'data-maptab-heading'
    );
  }
}])
  ...
  return {
    ...
    link: function(scope, elm, attrs, tabsetCtrl) {
      if (!scope.$eval(attrs.tabsetTitles)) {
        ...
        //now that tabs location has been decided, transclude the maptab titles in
 ...
 */
  ...
  return {
    ...
    link:function (originalScope, element, attrs, modelCtrl) {
      ...
      //bind keyboard events: arrows up(38) / down(40), enter(13) and maptab(9), esc(27)
